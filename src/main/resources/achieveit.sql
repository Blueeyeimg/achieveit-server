...
CREATE TABLE `activity` (
  ...
  PRIMARY KEY (`primary_activity`,`secondary_activity`),
  KEY `primary_activity` (`primary_activity`),
  KEY `secondary_activity` (`secondary_activity`)
...
CREATE TABLE `timesheet` (
  ...
  CONSTRAINT `timeheet_fk_5` FOREIGN KEY (`primary_activity`) REFERENCES `activity` (`primary_activity`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `timeheet_fk_6` FOREIGN KEY (`secondary_activity`) REFERENCES `activity` (`secondary_activity`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
